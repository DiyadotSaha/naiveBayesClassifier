...
# Student Name: Diya Saha
# UW Email    : dsaha4@ucsc.edu
...
class NaiveBayes():
    ...
    def word_set(self, filename: str):
        """ 
    ...
    def fit(self, train_hams: list, train_spams: list):
        ...
        def get_counts(filenames: list):
            ...
            """
            pass
        list_spam_email_words = []
        list_ham_email_words = []
        #collecting and making a list of list for all the words from all the spam emails
        for i in train_spams:
            temp_word_set = {}
            temp_word_set = self.word_set(i)
            list_spam_email_words += [temp_word_set]
        #collecting and making a list of list for all the words from all the ham emails
        for i in train_hams:
            temp_word_set = {}
            temp_word_set = self.word_set(i)
            list_ham_email_words += [temp_word_set]

        #creating the dictionary with word occurances for the spam email words
        for i in list_spam_email_words:
            for j in i:
                if j not in self.word_counts_spam:
                    self.word_counts_spam[j] = 1
                else:
                    self.word_counts_spam[j] += 1
        #creating the dictionary with word occurances for all the ham email words
        for i in list_ham_email_words:
            for j in i:
                if j not in self.word_counts_ham:
                    self.word_counts_ham[j] = 1
                else:
                    self.word_counts_ham[j] += 1
        #storing the num spam emails and num ham emails
        self.num_train_hams = len(list_ham_email_words)
        self.num_train_spams = len(list_spam_email_words)
        #print("num_train_hams: ", self.num_train_spams)
        #print("num_train_spams: ", self.num_train_hams)
    ...
    def predict(self, filename:str):
        ...
        """
        #probability of spam emails and ham emails
        prob_spam = np.log(self.num_train_spams / (self.num_train_hams+self.num_train_hams))
        prob_ham = np.log(self.num_train_hams / (self.num_train_hams+self.num_train_hams))
        #collecting all the words from the given email file
        words_in_email = self.word_set(filename)
        #calculating the overall probability of spam and ham emails
        for i in words_in_email:
            count_spam_word = self.word_counts_spam.get(i, 0)
            prob_spam += np.log((count_spam_word + 1) / (self.num_train_spams + 2))
            count_ham_word = self.word_counts_ham.get(i, 0)
            prob_ham += np.log((count_ham_word + 1) / (self.num_train_hams + 2))
        #if the probabilty of spam is higher we are going to label the email SPAM 
